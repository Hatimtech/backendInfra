...
exports.createBank = async (req, res) => {
    ...
    const {
        ...
        if(createUserResponse.length !== 0 ){
            res.send({ code: 0, message: "Error creating Bank"});
        }
        else {
...
exports.enableOrDisableBank = async (req, res) => {
    ...
    let editParams = {
        ...
        const editUserResponse = await editusers(token,userId,editParams);
        if(editUserResponse.length !== 0 ){
            res.send({ code: 0, message: "Bank Cannot be edited."});
        }
        else {
                res.send({code: 1,message: `Bank ${isEnabled ? "enabled" : "disabled"} successfully`});
